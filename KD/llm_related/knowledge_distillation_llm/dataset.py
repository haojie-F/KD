...
""" 
Alpaca
字段有 instruction、input、output
Belle
字段有 instruction、input、output
Stanford Self-Instruct
字段有 instruction、input、output
ShareGPT（需格式转换）
转换为上述字段即可
OpenOrca、UltraChat、WizardLM、LLaMA-2-Chinese（部分需格式转换）
只要能整理为 instruction/input/output 字段即可
自定义指令微调数据集
只要是类似结构的 json 文件均可直接使用
"""
